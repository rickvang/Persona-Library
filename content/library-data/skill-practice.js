...
Object.assign(window.PersonaLibraryDataFragments, {
  skillPractice: {
    ...
    'skill-architecture-decision-making': {
      operation:{
        startsWith:'An architecturally significant choice that changes system structure, a key quality attribute, or the cost of future change.',
        loop:['State the decision, outcome, requirements, constraints, and quality attributes before naming a preferred technology.','Identify the smallest credible alternatives, including keeping the current approach when it remains sufficient.','Compare tradeoffs across complexity, performance, reliability, security, cost, maintainability, lock-in, reversibility, and team capability only where they materially apply.','Use a proof, benchmark, model, or production evidence when a consequential uncertainty cannot be resolved from existing evidence.','Record the selected option, rejected alternatives, confidence, consequences, owner, and explicit trigger for reconsideration.','When the decision changes, supersede the prior record instead of rewriting its historical rationale.'],
        inputs:['Problem statement and intended outcome','Functional and nonfunctional requirements','Constraints, current architecture, and team capability','Evidence from production, documentation, tests, proofs, or benchmarks where material'],
        decisions:['Which choices are architecturally significant enough to record','Which alternatives are genuinely viable for the current requirement','Which uncertainty needs evidence before commitment','Whether the simplest current architecture still satisfies the need','What condition would justify reopening or superseding the decision'],
        outputs:['A selected architecture direction or explicit defer/no-change decision','An architecture decision record with alternatives and tradeoffs','Confidence, consequences, owner, and revisit trigger'],
        feedback:['Review the decision with the implementation and affected domain owners.','Compare observed implementation outcomes with the expected quality attributes.','Reopen the decision only when its stated assumptions or revisit conditions materially change.'],
        boundaries:'This capability chooses and records architecture direction. It does not replace product ownership, domain expertise, security review, UX judgment, or implementation validation.',
        leavesBehind:'A concise, inspectable architecture decision with enough context to understand why it applied and when it should be reconsidered.',
        moves:['Frame the decision before the technology.','Compare the smallest credible options.','Make tradeoffs and reversibility explicit.','Gather evidence for consequential uncertainty.','Record confidence, consequences, and revisit conditions.']
      },
      quality:{
        signals:['The decision is traceable to requirements and constraints rather than preference or trend.','Rejected alternatives are plausible and their tradeoffs are stated fairly.','The selected option is no more complex than the current need justifies.','Confidence and unresolved uncertainty are visible.','A future maintainer can tell what evidence would supersede the decision.'],
        checks:['Ask whether doing nothing or delaying infrastructure was considered when credible.','Trace each claimed benefit or risk to a requirement, constraint, evidence source, or explicit assumption.','Check whether the record distinguishes reversible implementation detail from hard-to-reverse architecture.','Verify that changed decisions supersede rather than silently rewrite accepted history.'],
        watchFor:['Technology is chosen before the problem and quality attributes are defined.','A long option list creates ceremony without changing the decision.','Future scale is asserted without a measurable trigger.','An ADR becomes a vendor tutorial instead of a decision record.','Low-confidence decisions are presented as settled facts.']
      }
    },
    'skill-web-application-architecture': {
      operation:{
        startsWith:'A web application, route, or feature needs a runtime structure that stays reliable across rendering, state, content, devices, performance, accessibility, and future change.',
        loop:['Start from the user task, approved interaction intent, content/data contract, and runtime constraints.','Map where code executes, how data moves, where state lives, and what can fail.','Choose the simplest rendering, server/client, routing, component, state, caching, and dependency boundaries that satisfy the requirement.','Implement or prototype the riskiest boundary with realistic content and failure states.','Validate browser behavior, accessibility implementation, build/test behavior, and performance at the cheapest sufficient layer.','Record consequential tradeoffs and revisit only when measured requirements change.'],
        inputs:['Product and UX intent','Representative content and data contract','Browser/device/network/hosting constraints','Accessibility and performance requirements','Existing code, framework, dependencies, tests, and production evidence'],
        decisions:['What must execute on the server, client, edge, or another runtime','Where component, route, state, data-access, caching, and error boundaries belong','Which shared abstraction reduces real duplication versus hiding context','Whether a framework feature or dependency is needed at all','Which quality claim requires field, browser, or production evidence'],
        outputs:['A proportionate frontend/runtime architecture','Explicit component, rendering, state, and dependency contracts','Verified implementation evidence and known untested conditions','Architecture notes for consequential choices'],
        feedback:['Run representative browser and content scenarios.','Inspect performance and accessibility evidence after material runtime changes.','Compare implementation behavior with design intent and data contracts.','Use regressions and production evidence to refine boundaries rather than adding architecture preemptively.'],
        boundaries:'This capability owns web-runtime engineering judgment. It does not decide unresolved product/task semantics, visual-design direction, or persistent data/source-of-truth architecture.',
        leavesBehind:'A web application whose runtime responsibilities and quality tradeoffs can be explained, tested, and changed without guessing.',
        moves:['Map execution and data flow.','Prefer the simplest rendering and state model.','Keep dependencies and provider details behind justified seams.','Implement semantic and resilient behavior.','Validate with realistic browser, accessibility, performance, and failure evidence.']
      },
      quality:{
        signals:['Server/client and state boundaries have a reason tied to the workload.','Realistic content, errors, loading, recovery, and responsive conditions do not break the architecture.','Accessibility semantics and interaction behavior survive implementation.','Performance claims use appropriate measurement rather than local intuition.','Framework or dependency choices can be revisited without rewriting unrelated application concerns.'],
        checks:['Trace every client boundary and long-lived state store to a capability that needs it.','Exercise long/empty/error/loading content across representative viewports and interaction methods.','Run available build, type, unit/integration, browser, accessibility, and performance checks proportionate to the change.','Inspect whether a new abstraction or dependency removes repeated responsibility or only hides it.'],
        watchFor:['Everything interactive becomes client-side by default.','Component reuse overrides task-specific behavior or semantics.','Local Lighthouse or one fast machine is presented as production performance proof.','Framework novelty is mistaken for architectural value.','The frontend silently owns database, authorization, or UX decisions outside its boundary.']
      }
    },
    'skill-application-and-data-architecture': {
      operation:{
        startsWith:'Application information needs a clear source of truth, content/domain model, persistence boundary, service contract, authorization model, migration path, or scaling decision.',
        loop:['Name the domain or content concepts, owners, producers, consumers, lifecycle, and invariants before choosing storage.','Define durability, consistency, latency, privacy, authorization, recovery, and editorial/operational requirements only to the level the use case needs.','Compare the least-complex viable persistence paths, including files or structured content when a database is not justified.','Model relationships and service/data-access contracts independently of presentation and provider syntax where replacement or reuse matters.','Plan schema/content-model evolution with compatibility, migration, validation, rollback, and observability.','Record scaling or provider-change triggers and revisit only when evidence crosses them.'],
        inputs:['Product and editorial workflows','Domain/content examples and edge cases','Read/write actors, ownership, authentication and authorization needs','Durability, consistency, latency, scale, privacy, and recovery requirements','Current storage/services and migration constraints'],
        decisions:['Which source is authoritative for each kind of information','Whether persistence is needed and what class of storage best fits the lifecycle','Which relationships and invariants belong in the model versus the presentation','Where API/service/data-access and authorization boundaries belong','How schema evolution remains compatible and recoverable','When caching, replication, queues, or provider abstraction solve a measured need rather than a hypothetical one'],
        outputs:['An explicit source-of-truth and domain/content model','A proportionate persistence and service architecture','Authorization and data-access contracts where required','A versioned migration/evolution plan for consequential changes','Scaling and provider-revisit triggers'],
        feedback:['Review the model with product/content and consuming implementation owners.','Test representative records, relationships, access rules, and failure/recovery paths.','Rehearse consequential migrations before production cutover.','Compare observed performance, reliability, and operating burden with the assumptions in the architecture decision.'],
        boundaries:'This capability owns application information, persistence, service, authorization, and evolution architecture. It does not own visual/interface design or browser/runtime implementation details.',
        leavesBehind:'A clear and evolvable source-of-truth model, service boundary, and migration history that can survive technology changes.',
        moves:['Model meaning and ownership before storage.','Choose the least-complex persistence that meets real requirements.','Separate presentation from domain/content contracts.','Make authorization, consistency, and failure behavior explicit.','Treat schema/provider changes as versioned migrations with validation and rollback.']
      },
      quality:{
        signals:['Each important datum or content object has an explicit authority and lifecycle.','The model expresses product/content meaning rather than mirroring one screen or vendor schema.','Persistence complexity is justified by real durability, access, workflow, or scale requirements.','Authentication and authorization are distinct where access control matters.','Schema/provider changes have a compatibility, validation, and rollback path.','Caches, replicas, exports, and derived views do not silently become competing sources of truth.'],
        checks:['Ask whether files or structured content are still sufficient before adding a database or CMS.','Walk representative create/read/update/delete and editorial lifecycle paths across actors.','Inspect referential integrity, authorization, migration fixtures, and recovery behavior for consequential changes.','Trace every cache or derived store to its authority and invalidation rule.','Verify that scale assumptions have measurable triggers.'],
        watchFor:['A database or CMS is selected before information ownership and lifecycle are defined.','The data model copies UI fields or page layout directly.','Authentication is treated as sufficient authorization.','Provider abstraction is added without meaningful switching or testing benefit.','Migration means copying rows without validating relationships, permissions, or business invariants.','Future scale is used to justify present operational complexity without evidence.']
      }
    }
  }
});
