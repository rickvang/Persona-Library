...
- `dist/job-tracker.html` is the top-level Applications companion surface. `content/job-tracker-page.html` and `client/job-tracker.js` own its UI and browser runtime. Real opportunity/application records live only in browser-local private state under the versioned tracker contract; they are not canonical Persona-Library records or committed repository content. The tracker uses portable URLs and a versioned JSON export so the surface can be extracted into a standalone private app without migrating Persona or Skill identities.
...
Keep the static-first architecture while canonical library content is primarily authored in Git. The Applications companion may use browser-local private state because its records are explicitly non-canonical and user-owned. Add authenticated/server-backed persistence only when application tracking needs cross-device sync, collaboration, accounts, permissions, or another requirement that local export/import cannot satisfy. Keep ordinary page state in the URL-aware helper until state must persist across users or sessions.
