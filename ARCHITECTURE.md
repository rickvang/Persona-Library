...
Callable Skills also carry a routing layer in their package metadata: `persona_applied` applies a capability to a target problem, `library_management` maintains system records, `orchestration` coordinates people and stages, `governance` reviews placement and downstream effects, and `tool_safety` resolves or exercises tool capabilities safely. The layer is a routing and ownership signal; it does not change the distinction between a Persona capability, a normalized Skill profile, a Tool, a workflow, or a Playbook. Non-callable workflows, templates, references, Decisions, prototypes, and Work Orders are classified by `artifact_kind` in the orientation manifest. Keep the repository package layout flat until loader support for nested namespaces is proven.
