...
- `dist/templates.html` is the Templates browse space: it catalogs reusable starting artifacts and scaffolds, shows purpose, applicability, source/location, status, evidence, revision, and scoped relationships without executing or rendering external artifacts. `dist/template.html` is the focused Template viewer, reached with a stable Template ID, and presents the same normalized record plus an explicit artifact-preview availability state.
