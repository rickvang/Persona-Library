...
- `content/library-data.js` owns authored personas, workflow maps, skill profiles, resource trails, modular skill guidance and practice overrides, primitive skill units, typed skill relationships, minimal Playbook identity and Template catalog records, Operating Pack catalog records and scoped applications, tool-use recipes, and explicit persona tool requirements.
...
- `dist/data/*.js`, `dist/data/site-orientation.json`, `dist/data/orientation/*.json`, and `dist/js/*.js` are generated output consumed by the Site pages and read-only consumers.
...
- `eval/` is the repository-local conformance layer. It owns concise fixtures, the normalized result contract, file-based evaluation, and recorded-result adapters. It records conditions and evidence; it does not imply live provider access, credentials, telemetry, or a production evaluation backend.
