...
Make the repository-owner approval Gate explicitly contingent on recorded review of the prior attempt's inactive state, changed files, and terminal CI state; keep the replacement held until each item and the owner's approval are evidenced. Rerun the isolated Riley/Noor conformance case, inspect latest-head validation and review state on PR #200, and keep #197 open until every acceptance criterion passes. Provider-level runtime lifecycle remains unobservable.