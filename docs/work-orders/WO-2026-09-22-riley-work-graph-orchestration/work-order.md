...
The read-only change-impact reconciliation is recorded in `runtime-dispatch-proof.md`. Open a reviewable PR for the Work Order/proof and scenario evidence update; use PR CI as repository validation, complete the representative Riley conformance review, and address scoped review findings. Keep #197 open until all acceptance criteria pass; do not claim the untested runtime replacement path.
