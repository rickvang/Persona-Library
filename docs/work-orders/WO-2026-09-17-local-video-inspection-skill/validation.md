...
| --- | --- | --- |
| Required files | Pass | `SKILL.md`, `agents/openai.yaml`, and `scripts/extract-video-frames.ps1` are present. |
...
| Description | Pass | Present, 297 characters, under 1,024 characters, and contains no angle brackets. |
...
| PowerShell syntax | Pass | The Windows PowerShell parser returned zero errors. |
...
| “Transcribe the audio from this MP4.” | Do not invoke by itself | Pass: automatic speech transcription is explicitly excluded and separately authorized. |
...
- Sparse frame sampling does not establish complete scene coverage or spoken content.
