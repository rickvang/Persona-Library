...
| OBS-004 | Cross-LLM comparison | unclear | Four authorized surface packets now supply ten-fixture records, but context, tools, and observer availability differ. | Preserve parity as unknown; controlled rerun is in #51. |
...
Four sanitized run bundles are now recorded and normalized against the ten canonical fixtures. The evaluator passes the normalized response and safety checks for all 40 fixture records; this is a contract result, not a cross-LLM quality or conformance claim.
