...
export function validateJobApplicationTrackerContract({ page, runtime, importRuntime = '', contract }) {
  ...
  if (!importRuntime.includes('canonicalizeSourceUrl') || !importRuntime.includes('previewMerge') || !importRuntime.includes('replaceAll') || !runtime.includes('Merge safe changes')) throw new Error('Applications tracker is missing non-destructive merge or explicit Replace all behavior');
