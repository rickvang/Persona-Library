...
export function validateJobApplicationTrackerContract({ page, runtime, importRuntime = '', storeRuntime = '', configRuntime = '', contract }) {
  ...
  if (!runtime.includes("FORMAT, version: FORMAT_VERSION") || !runtime.includes('importFile.addEventListener') || !importRuntime.includes('Unsupported tracker format') || !importRuntime.includes('unsupported version')) throw new Error('Applications tracker runtime is missing versioned import/export portability behavior');
  ...
  if (!storeRuntime.includes('createLocalStorageOpportunityStore') || !storeRuntime.includes('createSupabaseOpportunityStore') || !storeRuntime.includes("client.schema(schema).from('opportunities')") || !storeRuntime.includes('shouldCreateUser: false') || !storeRuntime.includes('persistSession: true') || !storeRuntime.includes('localStorage.getItem(storageKey)')) throw new Error('Applications tracker storage adapter is missing local migration or authenticated Supabase behavior');
  ...
  if (!contract.includes('Authenticated private opportunity store') || !contract.includes('LocalStorageOpportunityStore') || !contract.includes('SupabaseOpportunityStore') || !contract.includes('versioned JSON') || !contract.includes('standalone application') || !contract.includes('seen-job deduplication contract') || !contract.includes('non-destructive merge')) throw new Error('Application tracker contract is missing authenticated privacy, portability, extraction, merge, adapter, or seen-job separation');
