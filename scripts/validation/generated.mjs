...
export function validateJobApplicationTrackerContract({ page, runtime, importRuntime = '', storeRuntime = '', configRuntime = '', contract }) {
  ...
  if (!runtime.includes("FORMAT, version: FORMAT_VERSION") || !runtime.includes('postingDate') || !importRuntime.includes('postingDate') || !runtime.includes('importFile.addEventListener') || !importRuntime.includes('Unsupported tracker format') || !importRuntime.includes('unsupported version')) throw new Error('Applications tracker runtime is missing versioned import/export portability behavior');
