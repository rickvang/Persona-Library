...
export function validateJobApplicationTrackerContract({ page, runtime, contract }) {
  ...
  if (!runtime.includes('persona-library-job-application-handoff') || !runtime.includes("payload?.operation !== 'upsert'") || !runtime.includes('recordIdentity(record)') || !runtime.includes('confirm(`') || !runtime.includes('history.replaceState')) throw new Error('Applications tracker is missing confirmed fragment handoff ingestion or deduplicating upsert behavior');
  if (/\bfetch\s*\(/.test(runtime)) throw new Error('Applications tracker MVP must not add remote persistence or network calls');
  if (!contract.includes('browser-local private state') || !contract.includes('versioned JSON') || !contract.includes('standalone application') || !contract.includes('seen-job deduplication contract') || !contract.includes('#handoff=<payload>') || !contract.includes('private transient artifact')) throw new Error('Application tracker contract is missing privacy, portability, extraction, seen-job separation, or packet handoff boundary');
...
export async function validateGeneratedOutputs(context) {
  ...
  if (decisionSource.source !== 'docs/decisions/records.json' || !Array.isArray(decisionSource.records) || !decision010?.corrections?.length || !decision011?.status_note?.includes('DEC-013') || !decision011?.status_note?.includes('DEC-014') || !decision013?.qualifies?.includes('DEC-011') || !decision014?.qualifies?.includes('DEC-011') || !decision015?.qualifies?.includes('DEC-008') || !normalized(decision015?.decision).includes('candidate baseline resume') || !decision016?.qualifies?.includes('DEC-011') || !decision016?.qualifies?.includes('DEC-014') || !normalized(decision016?.decision).includes('browser-local private state') || !decision017?.qualifies?.includes('DEC-016') || !normalized(decision017?.decision).includes('base64url handoff payload') || files.decisionOutput !== renderDecisionsPage(files.decisionTemplateSource, decisionSource.records)) throw new Error('Generated Decisions output is stale or its authored source/history links are invalid; run build-library.mjs');
  ...
  if (!decisionsPage.includes('DEC-016') || !decisionsPage.includes('Application tracking stays private and extractable')) throw new Error('Generated Decisions page must present DEC-016 application tracking boundary');
  if (!decisionsPage.includes('DEC-017') || !decisionsPage.includes('Application packets hand off lifecycle state through private URL fragments')) throw new Error('Generated Decisions page must present DEC-017 packet tracker handoff boundary');
  ...
  const packetWorkOrder = await context.readFile('docs/job-search/application-work-order-template.md');
  if (!packetWorkOrder.includes('Applications tracker record state') || !packetWorkOrder.includes('Tracker handoff status') || !packetWorkOrder.includes('Packet Ready') || !packetWorkOrder.includes('Applied') || !packetWorkOrder.includes('application-tracker-contract.md')) throw new Error('Application packet Work Order is missing tracker lifecycle handoff integration');
