...
test('Repository working copy and small-change lane replace the local-checkout ban without bypassing Work Graph supervision', () => {
  ...
  assert.throws(() => validateRepositoryWorkingCopyContract({ ...args, architecture: `${architecture} Work Orders are the repository-wide active-work packet and progress record for non-trivial work.` }), /automatic Work Orders/);
  assert.throws(() => validateRepositoryWorkingCopyContract({ ...args, architecture: `${architecture} Active non-trivial work stays in docs/work-orders/<work-order-id>/.` }), /placement guidance/);
...
test('Riley work graph contract keeps one authoritative dispatch, evidence gates, and read-before-retry recovery', () => {
  ...
  const operationalScenarioCatalog = [{
    ...
    status:'active',
    expectedRoute:'Current Work and applicable recovery artifact reconciliation',
    preconditions:['Use a linked Work Order when one exists, otherwise the smallest Verification Queue or domain artifact.'],
  ...
  const skillsRoute = {routes:[{id:'work-graph-orchestration',target:'work-graph-orchestration',package_path:'.agents/skills/work-graph-orchestration',first_reads:['Current Work row when the workstream already exists','linked Work Order when one exists, otherwise the smallest authoritative issue / PR / Verification Queue / domain artifact']}]};
  const skillPackage = 'Keep one authoritative active Dispatch per WorkNode. Distinguish handoff from supervised delegation. Parallelize only collision-safe work. Recovery is read-before-retry. Use the Execution-adapter contract. This Skill does not persist a second canonical task database. A bounded correction stays in the same attempt: CI runs and review events are not new Dispatches. Create a new Dispatch identity only when the attempt is abandoned, superseded, or reassigned. The small-change lane changes tracking artifacts, not orchestration membership, when existing records already hold enough recovery state. Rehydrate from the linked Work Order when one exists, otherwise the smallest authoritative issue/PR/Verification Queue/domain artifact.';
  ...
  assert.throws(() => validateRileyWorkGraphContract({ riley, rileyFlows, skillCatalog, operationalScenarioCatalog:[{...operationalScenarioCatalog[0],preconditions:['Current Work / Work Order checkpoints are available.'],expectedRoute:'Current Work and Work Order reconciliation'}], skillsRoute, skillPackage }), /optional-Work-Order hierarchy/i);
  assert.throws(() => validateRileyWorkGraphContract({ riley, rileyFlows, skillCatalog, operationalScenarioCatalog, skillsRoute: {routes:[{...skillsRoute.routes[0],first_reads:['Current Work row','linked Work Order']}]}, skillPackage }), /orientation.*conditional/i);
  assert.throws(() => validateRileyWorkGraphContract({ riley, rileyFlows, skillCatalog, operationalScenarioCatalog, skillsRoute, skillPackage: skillPackage.replace('linked Work Order when one exists, otherwise the smallest authoritative issue/PR/Verification Queue/domain artifact', 'linked Work Order') }), /Callable work-graph Skill/i);
});
