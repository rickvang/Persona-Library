...
12. For work that depends on current repository state, treat `rickvang/Persona-Library` as this project's canonical GitHub remote unless the requester explicitly names another repository. Prefer an exposed connected/native GitHub plugin or connector for supported operations; use browser or computer interaction only for unsupported operations, explicitly requested UI workflows, or an accepted fallback. Refresh current `main` and relevant issue, pull request, branch, commit, review, and check state before planning or review, and again before consequential mutations. If the direct integration is available but not connected, ask the requester to connect or authorize it; if GitHub is unavailable, report that limitation instead of inferring remote state. Follow the pinned [GitHub operating instructions](https://github.com/rickvang/tool-repo/blob/94acc6082e941439d2ee532f1b1b091cd42eb923/tools/github/AGENTS.md). Reading GitHub state does not authorize comments, edits, branch or file writes, pull request changes, merges, closes, labels, assignments, settings, or access changes.
13. For bounded parallel implementation across independent repository workstreams, use the [Bounded Parallel Implementation Playbook](docs/bounded-parallel-implementation-playbook.md). Keep the coordinator thin, give each implementer one workstream, one branch, and one PR then STOP, pass compact GitHub-reference handoffs, review from current GitHub state, and treat merge as a separately authorized mutation. Do not nest implementer agents or copy child transcripts into the coordinator.
