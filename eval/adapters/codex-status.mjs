...
function normalizeWindow(window) {
  ...
  if (window.value !== undefined || window.unit !== undefined || window.mode !== undefined) {
    if (candidates.length) throw new Error(`Codex status window ${window.window_id || '(unknown)'} mixes normalized and convenience fields.`);
    return { ...base, unit: window.unit, mode: window.mode, value: window.value };
  }
  if (candidates.length !== 1) throw new Error(`Codex status window ${window.window_id || '(unknown)'} must provide exactly one supported value field.`);
  const [field, unit, mode] = candidates[0];
  return { ...base, unit, mode, value: window[field] };
}

export function normalizeCodexStatusSnapshot(input) {
  if (!input || typeof input !== 'object') throw new Error('Codex status input must be an object.');
  const snapshot = {
    source: input.source || 'Codex /status',
